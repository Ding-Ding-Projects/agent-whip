...
Run these via the shell tool available in this environment, exactly as written. Full flag
reference and exit codes are in [`references/usage.md`](references/usage.md); the
identity-resolution and audit-log guarantees that make firing with no confirmation prompt safe are
in [`references/safety.md`](references/safety.md).
...
- **This skill is never invoked implicitly.** Cracking a session changes its operating mode and,
  at tier 2, hands it a cleanup authorization — that is a deliberate human action, never something
  an agent should trigger on itself or on a sibling session. See `agents/openai.yaml`.
